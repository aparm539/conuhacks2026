 ...
 */
function createFileLevelFallbackRange(
  document: vscode.TextDocument,
): vscode.Range {
...
function filterContextsByFile(
  ...
  console.warn(
    `No context snapshots found for file ${currentFile}, using any available context`,
  );
  return contexts;
}
...
export async function extractCodeContext(
  ...
  if (context.symbolsInView && context.symbolsInView.length > 0) {
    try {
      ...
      console.warn(
        "Failed to query document symbols for code extraction:",
        error,
      );
    }
  }
  ...
  if (
    ...
    if (lines.length > 0) {
      codeSnippets.push(
        `Cursor context (line ${context.cursorLine + 1}):\n${lines.join("\n")}`,
      );
    }
  }
  ...
  if (
    codeSnippets.length === 0 &&
    context.visibleRange[0] >= 0 &&
    context.visibleRange[1] < lineCount
  ) {
...
export async function findCommentLocationsBatch(
  ...
  try {
    ...
    if (locations.length !== segments.length) {
      throw new Error(
        `Invalid response: expected ${segments.length} locations, got ${locations.length}`,
      );
    }
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (
        selectedCandidate.cursorLine >= 0 &&
        selectedCandidate.cursorLine < lineCount
      ) {
      ...
      // Try visible symbols
      if (
        selectedCandidate.symbolsInView &&
        selectedCandidate.symbolsInView.length > 0
      ) {
        try {
          const symbols = await vscode.commands.executeCommand<
            vscode.DocumentSymbol[]
          ...
          console.warn(
            "Failed to query document symbols for comment placement:",
            error,
          );
        }
      }
      ...
      if (
        selectedCandidate.visibleRange[0] >= 0 &&
        selectedCandidate.visibleRange[0] < lineCount
      ) {
        ranges.push(
          createLineRange(document, selectedCandidate.visibleRange[0]),
        );
        continue;
      }
    ...
    console.warn(
      "Failed to use Gemini for location selection, falling back to simple location:",
      error,
    );
    // Fallback: use cursor line from nearest context
    const ranges: vscode.Range[] = [];
    for (const segment of segments) {
      const nearestContexts = findNearestContexts(
        segment.startTime,
        contexts,
        1,
        currentFile,
      ...
      if (
        nearestContexts.length > 0 &&
        nearestContexts[0].cursorLine >= 0 &&
        nearestContexts[0].cursorLine < lineCount
      ) {
