...
export class ContextCollector {
	...
	startRecording(): void {
		...
		const activeEditorDisposable = vscode.window.onDidChangeActiveTextEditor(() => {
			this.collectContextPromise = this.collectContextPromise.then(() => this.collectContext());
		});
		...
		const visibleRangesDisposable = vscode.window.onDidChangeTextEditorVisibleRanges((e) => {
	...
	private async collectContext(): Promise<void> {
		...
		try {
			...
			if (symbols && Array.isArray(symbols)) {
				symbolsInView = this.filterSymbolsInRange(symbols, visibleRange[0], visibleRange[1]);
			}
		} catch (error) {
	...
	private filterSymbolsInRange(
		...
		for (const symbol of symbols) {
			...
			if (symbol.children && symbol.children.length > 0) {
				result.push(...this.filterSymbolsInRange(symbol.children, startLine, endLine));
			}
		}
