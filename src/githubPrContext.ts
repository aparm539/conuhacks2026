...
function getConfigOverrides(): { owner?: string; repo?: string; pullNumber?: number } {
	...
	return {
		...
		pullNumber: typeof pullNumber === 'number' && pullNumber > 0 ? pullNumber : undefined,
	};
}
...
function getGitInfoFromApi(root: string): GitInfo | null {
	try {
		const ext = vscode.extensions.getExtension<{ getAPI: (v: number) => unknown }>('vscode.git');
		if (!ext?.isActive) {
			return null;
		}
		type GitRepo = {
			rootUri: { fsPath: string };
			state?: { HEAD?: { name?: string }; remotes?: { name?: string; fetchUrl?: string }[] };
		};
		const api = ext.exports?.getAPI(1) as { repositories?: GitRepo[] } | undefined;
		if (!api?.repositories?.length) {
			return null;
		}
		const repo = api.repositories.find((r: GitRepo) => {
			const rp = r.rootUri.fsPath;
			return root === rp || root.startsWith(rp + '/') || rp.startsWith(root + '/');
		}) ?? api.repositories[0];
...
function getGitInfoFromCli(root: string): GitInfo | null {
	try {
		const remoteUrl = execSync('git remote get-url origin', { encoding: 'utf8', cwd: root }).trim();
 ...
 */
export async function getPrContext(token: string | undefined): Promise<PrContext | null> {
	const overrides = getConfigOverrides();

	if (overrides.owner && overrides.repo && overrides.pullNumber) {
		const commitId = token
			? await fetchPrDetails(token, overrides.owner, overrides.repo, overrides.pullNumber)
			: null;
		if (!commitId) {
			return null;
		}
	...
	if (overrides.pullNumber) {
		const commitId = await fetchPrDetails(token, owner, repo, overrides.pullNumber);
		if (!commitId) {
			return null;
		}
