...
Classifications:`;
  ...
  const validClassifications: SegmentClassification[] = ['Ignore', 'Question', 'Concern', 'Suggestion', 'Style'];
  ...
  for (let i = 0; i < classifications.length; i++) {
    if (!validClassifications.includes(classifications[i] as SegmentClassification)) {
 ...
 */
async function classifySegmentsSequentially(
  segments: SpeakerSegment[]
 ...
 */
async function transformSegmentsSequentially(
  classifiedSegments: ClassifiedSegment[]
      ...
      if (parsed.length === 1 && Array.isArray(parsed[0])) {
        ...
        if (inner.length === segments.length) {
          ...
          const isValid = inner.every(item => 
            item === 'keep' || (Array.isArray(item) && item.every(part => typeof part === 'string'))
          );
          if (isValid) {
 ...
 */
async function splitSegmentsSequentially(
  classifiedSegments: ClassifiedSegment[]
): Promise<ClassifiedSegment[]> {
 ...
 */
async function selectCommentLocation(
  commentText: string,
  classification: SegmentClassification,
  candidates: CandidateLocation[],
  fileName: string
...
app.post('/classify', async (req: Request<{}, ClassificationResponse, ClassificationRequest>, res: Response<ClassificationResponse>) => {
  try {
    if (!geminiClient) {
      return res.status(500).json({ 
        classifiedSegments: [],
...
app.post('/transform', async (req: Request<{}, TransformationResponse, TransformationRequest>, res: Response<TransformationResponse>) => {
  try {
    if (!geminiClient) {
      return res.status(500).json({ 
        transformedSegments: [],
...
app.post('/split', async (req: Request<{}, SplitResponse, SplitRequest>, res: Response<SplitResponse>) => {
  try {
    if (!geminiClient) {
      return res.status(500).json({ 
        splitSegments: [],
...
app.post('/select-comment-location', async (req: Request<{}, LocationSelectionResponse, LocationSelectionRequest>, res: Response<LocationSelectionResponse>) => {
