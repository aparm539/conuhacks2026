 ...
 */
async function getDiarization(audioBuffer) {
    const tempFilePath = path_1.default.join(os_1.default.tmpdir(), `audio-${Date.now()}.wav`);
    try {
        // Write audio buffer to temporary file
        fs_1.default.writeFileSync(tempFilePath, audioBuffer);
        // Create FormData for multipart/form-data upload
        const formData = new form_data_1.default();
        formData.append('audio', fs_1.default.createReadStream(tempFilePath), {
            filename: 'audio.wav',
            contentType: 'audio/wav'
        });
        // Call diar-service using axios (better form-data support)
        const response = await axios_1.default.post(`${DIAR_SERVICE_URL}/process`, formData, {
 ...
 */
function matchWordsToSpeakers(words, diarizationSegments) {
    ...
    const getSpeakerNumber = (speakerLabel) => {
    ...
    return words.map(word => {
        ...
        if (!bestSegment && sortedSegments.length > 0) {
            ...
            for (const segment of sortedSegments) {
                const distance = Math.min(Math.abs(wordStartTime - segment.start), Math.abs(wordStartTime - segment.end));
                ...
                try {
                    ...
                    if (altResponse.results && altResponse.results.length > 0) {
                        ...
                        if (alternative && alternative.words) {
                            ...
                            const wordsWithoutSpeakers = alternative.words.map((wordInfo) => ({
                                word: wordInfo.word || '',
